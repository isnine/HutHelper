...
- (IBAction)Buuton1:(id)sender {
    ...
    NSString *Url_String=[NSString stringWithFormat:@"http://218.75.197.121:8888/api/v1/stuff/detail/%@/%@/%@",user.data.studentKH,user.remember_code_app,[self getid:(short)([self getIndexPath].section+1)*2-1]];
...
- (IBAction)Button2:(id)sender {
    ...
    NSDictionary *User_Data=[defaults objectForKey:@"User"];
    User *user=[User yy_modelWithJSON:User_Data];
    NSString *Url_String=[NSString stringWithFormat:@"http://218.75.197.121:8888/api/v1/stuff/detail/%@/%@/%@",user.data.studentKH,user.remember_code_app,[self getid:(short)([self getIndexPath].section+1)*2]];
    NSLog(@"商品查询地址:%@",Url_String);
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    [manager.requestSerializer willChangeValueForKey:@"timeoutInterval"];
    manager.requestSerializer.timeoutInterval = 4.f;
