...
- (void)viewDidLoad {
  ...
  if (self.isSelfGoods) {
    ...
    UIView *rightButtonView =
        [[UIView alloc] initWithFrame:CGRectMake(0, 0, 100, 50)];
    UIButton *mainAndSearchBtn =
        [[UIButton alloc] initWithFrame:CGRectMake(70, 0, 50, 50)];
    [rightButtonView addSubview:mainAndSearchBtn];
    [mainAndSearchBtn setImage:[UIImage imageNamed:@"ico_hand_delete"]
                      forState:UIControlStateNormal];
    [mainAndSearchBtn addTarget:self
                         action:@selector(delectGoods)
    ...
    UIButton *mainAndSearchBtn =
        [[UIButton alloc] initWithFrame:CGRectMake(70, 0, 50, 50)];
    [rightButtonView addSubview:mainAndSearchBtn];
    [mainAndSearchBtn setImage:[UIImage imageNamed:@"ico_user_user"]
                      forState:UIControlStateNormal];
    [mainAndSearchBtn addTarget:self
...
- (void)setHeadImg {
  //背景放大并高斯模糊
  UIImageView *backImgView = [[UIImageView alloc]
      initWithFrame:CGRectMake(0, 0, DeviceMaxWidth,
                               SYReal(390) + iphoneX_Y(113))];
  //中心切割
  ...
  UIToolbar *toolbar = [[UIToolbar alloc]
      initWithFrame:CGRectMake(0, 0, backImgView.frame.size.width,
                               backImgView.frame.size.height)];
  ...
  for (int i = 0; i < goodsImgArray.count && i < 3; i++) {
    UIImageView *goodsImgView = [[UIImageView alloc]
        initWithFrame:CGRectMake(SYReal(img_x), SYReal(110) + iphoneX_Y(60),
                                 SYReal(120), SYReal(120))];
    //中心切割
    ...
    UITapGestureRecognizer *tapGestureRecognizer =
        [[UITapGestureRecognizer alloc]
            initWithTarget:self
                    action:@selector(scanBigImageClick:)];
...
- (void)setText {
  //标题
  UILabel *goodsTitleLab = [[UILabel alloc]
      initWithFrame:CGRectMake(SYReal(26), SYReal(245) + iphoneX_Y(113),
                               SYReal(363), SYReal(40))];
  ...
  //内容
  UITextView *goodsContentText = [[UITextView alloc]
      initWithFrame:CGRectMake(SYReal(22), SYReal(280) + iphoneX_Y(113),
                               SYReal(368), SYReal(92))];
  ...
  for (int i = 0; i < 4; i++) {
    UILabel *Lab = [[UILabel alloc]
        initWithFrame:CGRectMake(SYReal(LabX[i]),
                                 SYReal(LabY[i]) + iphoneX_Y(113), SYReal(50),
                                 SYReal(30))];
  ...
  for (int i = 0; i < 4; i++) {
    UITextView *goodsLab = [[UITextView alloc]
        initWithFrame:CGRectMake(SYReal(LabX[i]),
                                 SYReal(LabY[i] + 40) + iphoneX_Y(113),
                                 SYReal(170), SYReal(80))];
...
- (void)setFoot {
  //绘制底部背景
  UIImageView *foodImgView = [[UIImageView alloc]
      initWithFrame:CGRectMake(0, SYReal(680) + iphoneX_Y(113), DeviceMaxWidth,
                               SYReal(56) + iphoneX_Y(113))];
...
- (void)delectGoods {
  UIAlertController *alert =
      [UIAlertController alertControllerWithTitle:@"删除商品"
                                          message:@"是否要删除当前商品"
...
- (void)user {
  UserShowViewController *userShowViewController =
      [[UserShowViewController alloc] init];
  ...
  [self.navigationController pushViewController:userShowViewController
                                       animated:YES];
}
#pragma mark - 数据
- (NSString *)getimg:(int)i {
  ...
  NSString *Url_String =
      [NSString stringWithFormat:@"%@/%@", [Config getApiImg], img[i]];
  return Url_String;
}
- (NSString *)getcontact {
  ...
  if (![[_handDic objectForKey:@"wechat"] isEqualToString:@""]) {
    return [NSString
        stringWithFormat:@"微信：%@", [_handDic objectForKey:@"wechat"]];
  }
  return @"";
}
#pragma mark - 代理方法
