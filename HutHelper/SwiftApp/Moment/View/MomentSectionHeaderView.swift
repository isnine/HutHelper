...
class MomentSectionHeaderView: UITableViewHeaderFooterView {
    ...
    lazy var timeLab:UILabel = {
       let label = UILabel()
        label.font = UIFont.init(name: "HelveticaNeue-Light", size: 9.fit)
    ...
    lazy var menuBtn:UIButton = {
       let btn = UIButton()
        btn.setImage(UIImage(named: "abc_ic_more_gray_22x22_"), for: .normal)
    ...
    lazy var contentLab:UITextView = {
       let textView = UITextView()
        textView.font = UIFont.systemFont(ofSize: 15.fit)
    ...
    lazy var contentBtn:UIButton = {
       let btn = UIButton()
        btn.setTitleColor(momentColor, for: .normal)
        btn.titleEdgeInsets = UIEdgeInsets(top: 0, left: 10.fit, bottom: 0, right: 0)
        btn.titleLabel?.font = UIFont.init(name: "HelveticaNeue-Light", size: 12.fitW)
        btn.rx.tap.subscribe(onNext:{[weak self] in
            self!.delegate?.cellClickCallBack(with: self!.cellData, type: .contentFold,indexPath: self!.indexPath)
            }).disposed(by: disposeBag)
        //label.backgroundColor = .red
        return btn
    }()
    ...
    // 学院
    lazy var depNameLabel:UILabel = {
       let label = UILabel()
        label.font = UIFont.init(name: "HelveticaNeue-Light", size: 12.fit)
        label.textColor = UIColor.init(r: 153, g: 153, b: 153)
        //label.backgroundColor = .red
        return label
    }()
    lazy var depNameBtn:UIButton = {
       let btn = UIButton()
        btn.setTitleColor(momentColor, for: .normal)
        btn.titleEdgeInsets = UIEdgeInsets(top: 0, left: 10, bottom: 0, right: 0)
        btn.titleLabel?.font = UIFont.init(name: "HelveticaNeue-Light", size: 12.fit)
        btn.rx.tap.subscribe(onNext:{[weak self] in
            self!.delegate?.cellClickCallBack(with: self!.cellData, type: .fold,indexPath: self!.indexPath)
            }).disposed(by: disposeBag)
        //label.backgroundColor = .red
        return btn
    }()
    ...
    lazy var commentBtn:UIButton = {
       let btn = UIButton()
        ...
        btn.titleLabel?.font = UIFont.systemFont(ofSize: 14.fit)
        btn.imageEdgeInsets = UIEdgeInsets(top: 0, left: -15.fit, bottom: 0, right: 0)
    ...
    lazy var deleteBtn:UIButton = {
       let btn = UIButton()
        ...
        btn.titleLabel?.font = UIFont.systemFont(ofSize: 12.fitW)
...
//        print("contentHeight",contentHeight)
        var contentHeight:CGFloat = data.contentHeight
        if contentHeight >= 250.fitW && data.conntentIsFold {
            contentHeight = 250.fit
            contentBtn.setTitle("展开", for: .normal)
            contentLabel.text = data.content
            contentLab.text = ""
        }else {

            contentBtn.setTitle("收起", for: .normal)
        ...
        if data.contentHeight < 250.fitW {
        ...
        sumHeight += (Int(contentHeight) + blank)//(Int(contentHeight+10).fitW + blank)
        // 图片布局
        var imgHeight:Int = 0
        ...
        depNameLabel.snp.remakeConstraints { (make) in
            make.left.equalTo(self).offset(20.fitW)
            make.width.equalTo(150.fitW)
            make.height.equalTo(20.fitW)
            make.top.equalTo(self).offset(sumHeight)
        }
        depNameBtn.snp.remakeConstraints { (make) in
            make.centerX.equalTo(self).offset(-20.fitW)
            make.width.equalTo(80.fitW)
            make.height.equalTo(20.fitW)
                       make.top.equalTo(self).offset(sumHeight)
        }
        ...
        sumHeight += (20.fitW + blank)
        if data.comments.count != 0 && !data.isFold {
            line.isHidden = false
        ...
        depNameLabel.text = data.dep_name
        if data.isFold {
            depNameBtn.setTitle("展开评论(\(data.comments.count))", for: .normal)
        }else {
            depNameBtn.setTitle("收起评论", for: .normal)
        }
        if data.comments.count == 0 {
            depNameBtn.setTitle("", for: .normal)
            depNameBtn.isUserInteractionEnabled = false
        }else {
            depNameBtn.isUserInteractionEnabled = true
        }
    ...
    func setUI(){
        ...
        addSubview(depNameLabel)
        
        addSubview(depNameBtn)
        ...
        depNameLabel.snp.makeConstraints { (make) in
            make.left.equalTo(self).offset(20.fitW)
            make.width.equalTo(150.fitW)
            make.height.equalTo(20.fitW)
        }
        depNameBtn.snp.makeConstraints { (make) in
            make.centerX.equalTo(self).offset(-20.fitW)
...
extension MomentSectionHeaderView:UICollectionViewDelegateFlowLayout, UICollectionViewDataSource,UICollectionViewDelegate{
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        //let browser = JXPhotoBrowser.init(dataSource: nil, delegate: nil, transDelegate: nil)
