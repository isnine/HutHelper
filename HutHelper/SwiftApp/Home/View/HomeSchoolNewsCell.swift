...
protocol HomeSchoolNewsCelldelegate:NSObjectProtocol {
    func cellClickCallBack(with data:HomeSchoolNewsModel)
}

class HomeSchoolNewsCell: UICollectionViewCell {
    
    var celldata = [HomeSchoolNewsModel]()
    // 代理回调
    weak var delegate : HomeSchoolNewsCelldelegate?
...
extension HomeSchoolNewsCell: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        delegate?.cellClickCallBack(with: celldata[indexPath.row])
    }
